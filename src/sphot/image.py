...
class SpotPerCellAnalyzer:
    ...
    def calculateECDFs(self):
        self.ecdfs = {}
        for label in range(1, self.maxLabel + 1):
            self.ecdfs[label] = self.getECDF(self.nnDistances[label][0])


    def getRandomPointsForLabel(self, label):
        nrOfPoints = len(self.pointsPerCell[label])
        label = np.ndarray.copy(self.labels)
        label[label < label] = 0
        label[label > label] = 0
        nz = np.nonzero(label)
        coords = np.transpose(nz)
        maxSample = len(coords) - 1
        indices = random.sample(range(maxSample + 1), nrOfPoints)
    ...
    def getEnvelopFor(self, label, nrOfSamples=100):
        ...
        for i in range(nrOfSamples):
            ...
            distances = self.getNNDistancesFor(points)[0]
            cdf = ecdf(distances)
